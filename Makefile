...
LAYA_IMAGE := laya-gpu:$(LAYA_VERSION)-$(TORCH_INDEX)
KEV_IMAGE := kev-gpu:$(KEV_REF)-$(TORCH_INDEX)
comma := ,
PROFILES := $(subst $(comma), ,$(COMPOSE_PROFILES))
...
.PHONY: help env build up down logs download gpu-check smoke

help:
	@echo "Servers come from COMPOSE_PROFILES in .env (now: $(COMPOSE_PROFILES))."
...
download: build
	...
	$(if $(filter laya,$(PROFILES)),$(FETCH) $(LAYA_IMAGE) -c \
	...
	$(if $(filter kev,$(PROFILES)),$(FETCH) $(KEV_IMAGE) -c \
...
smoke:
	@$(if $(filter laya,$(PROFILES)),$(call smoke-one,laya,$(LAYA_PORT),$(LAYA_API_KEY)))
	@$(if $(filter kev,$(PROFILES)),$(call smoke-one,kev,$(KEV_PORT),$(KEV_API_KEY)))

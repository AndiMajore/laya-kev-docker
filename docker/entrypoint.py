# Adapted from NandhaKishorM/laya (docker/entrypoint.py), Apache-2.0: adds KEV_API_KEY.
"""Load supported file-backed secrets, then replace this process with the command."""
import os
from pathlib import Path
import sys

# Each name is read as `<NAME>_FILE` and then moved into `<NAME>`, so a secret can be
# mounted as a file instead of passed in the environment.
SECRET_NAMES = ("HF_TOKEN", "LAYA_API_KEY", "KEV_API_KEY")
